...
import { CONFIG_GEOMETRIA, AJUSTES_PARALELOGRAMO } from "./ConstantesVisuales";
...
export const RenderizadorParalelogramo = ({ parametros }: Props) => {
  ...
  const escalaBase = AJUSTES_PARALELOGRAMO?.escalaFuente || 0.8;
  ...
  const etiquetasCalculadas = etiquetas.map((eti, i) => {
    const PADDING_DINAMICO = AJUSTES_PARALELOGRAMO?.distanciasTexto?.[i] || 0.6;
    return {
      texto: eti.texto,
  ...
  // Cuadradito de 90 grados para la altura
  const ladoCuadro = 0.4;
  const cuadro90 = [
    [finAltura[0] + ladoCuadro, finAltura[1]], // Hacia la derecha
