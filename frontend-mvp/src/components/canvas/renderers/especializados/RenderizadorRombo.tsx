import { useState } from "react";
import { Mafs, Line, Text, Polygon } from "mafs";
import { CONFIG_GEOMETRIA, AJUSTES_ROMBO } from "./ConstantesVisuales";

interface EtiquetaVectorial {
  texto: string;
  mx: number;
  my: number;
  nx: number;
  ny: number;
}

interface Props {
  parametros: {
    vertices: [number, number][];
    etiquetasDiagonales: EtiquetaVectorial[];
  };
}

export const RenderizadorRombo = ({ parametros }: Props) => {
  const [escalaTexto, setEscalaTexto] = useState(1);

  if (!parametros || !parametros.vertices) return null;
  const { vertices, etiquetasDiagonales } = parametros;

  const escalaBase = AJUSTES_ROMBO?.escalaFuente || 0.75;
  const tamanoBase = CONFIG_GEOMETRIA.TAMANO_TEXTO * escalaTexto * escalaBase;

  // 1. MOTOR DE NORMALIZACIÓN
  const xs = vertices.map((v) => v[0]);
  const ys = vertices.map((v) => v[1]);
  const ancho = Math.max(...xs) - Math.min(...xs);
  const alto = Math.max(...ys) - Math.min(...ys);
  const maxDim = Math.max(ancho, alto) || 1;
  const scale = 10 / maxDim;

  const verticesEscalados = vertices.map(
    (v) => [v[0] * scale, v[1] * scale] as [number, number],
  );

  // 2. CÁLCULO DE POSICIONES
  const etiquetasCalculadas = etiquetasDiagonales.map((eti, i) => {
    const PADDING_DINAMICO = AJUSTES_ROMBO?.distanciasTexto?.[i] || 0.5;
    return {
      texto: eti.texto,
      x: eti.mx * scale + eti.nx * PADDING_DINAMICO,
      y: eti.my * scale + eti.ny * PADDING_DINAMICO,
    };
  });

  // 3. CÁMARA DINÁMICA
  const minX = Math.min(
    ...verticesEscalados.map((v) => v[0]),
    ...etiquetasCalculadas.map((e) => e.x),
  );
  const maxX = Math.max(
    ...verticesEscalados.map((v) => v[0]),
    ...etiquetasCalculadas.map((e) => e.x),
  );
  const minY = Math.min(
    ...verticesEscalados.map((v) => v[1]),
    ...etiquetasCalculadas.map((e) => e.y),
  );
  const maxY = Math.max(
    ...verticesEscalados.map((v) => v[1]),
    ...etiquetasCalculadas.map((e) => e.y),
  );

  // 🔥 Margen ligeramente más suelto (0.8) para que los binomios inmensos no toquen el borde de la caja
  const MARGEN = 0.8;
  const viewBox = {
    x: [minX - MARGEN, maxX + MARGEN] as [number, number],
    y: [minY - MARGEN, maxY + MARGEN] as [number, number],
  };

  return (
    <div className="flex flex-col items-center w-full">
      <div className="flex justify-end w-full mb-2 gap-2 pr-4">
        <button
          onClick={() => setEscalaTexto((p) => Math.max(0.7, p - 0.1))}
          className="px-2 py-1 text-xs bg-slate-100 rounded text-slate-600 font-bold"
        >
          A-
        </button>
        <button
          onClick={() => setEscalaTexto((p) => Math.min(1.5, p + 0.1))}
          className="px-2 py-1 text-xs bg-slate-100 rounded text-slate-600 font-bold"
        >
          A+
        </button>
      </div>

      <div className="border border-slate-200 rounded-xl bg-white shadow-sm overflow-hidden flex justify-center items-center w-full p-2 h-[280px]">
        <Mafs height={270} viewBox={viewBox} pan={false} zoom={false}>
          <g>
            {/* Relleno Estético */}
            <Polygon
              points={verticesEscalados}
              color={CONFIG_GEOMETRIA.COLOR_LINEA_PRINCIPAL}
              fillOpacity={0.1}
              weight={2.5}
            />

            {/* Diagonales Punteadas */}
            <Line.Segment
              point1={verticesEscalados[2]}
              point2={verticesEscalados[0]}
              color={CONFIG_GEOMETRIA.COLOR_LINEA_SECUNDARIA}
              weight={2}
              style="dashed"
            />
            <Line.Segment
              point1={verticesEscalados[3]}
              point2={verticesEscalados[1]}
              color={CONFIG_GEOMETRIA.COLOR_LINEA_SECUNDARIA}
              weight={2}
              style="dashed"
            />

            {/* 🔥 Textos con "Halo Protector" (paintOrder) */}
            {etiquetasCalculadas.map((eti, i) => (
              <Text
                key={`eti-${i}`}
                x={eti.x}
                y={eti.y}
                size={tamanoBase}
                color="#000000"
                svgTextProps={{
                  fontWeight: "700",
                  textAnchor: "middle",
                  dominantBaseline: "middle",
                  // Esta línea es magia pura de UI: Pone un borde blanco grueso debajo del texto negro
                  style: {
                    paintOrder: "stroke",
                    stroke: "#ffffff",
                    strokeWidth: "6px",
                    strokeLinecap: "round",
                    strokeLinejoin: "round",
                  },
                }}
              >
                {eti.texto}
              </Text>
            ))}
          </g>
        </Mafs>
      </div>
    </div>
  );
};
