...
export default function ResultsPage() {
  ...
  if (loading)
    return (
      <div className="h-screen w-full flex items-center justify-center dark:bg-slate-950 dark:text-white font-black text-xl tracking-widest uppercase">
        Cargando...
      </div>
    );
  ...
  if (!result || !result.questions)
    return (
      <div className="p-10 w-full text-center font-bold">
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors flex flex-col w-full m-0 p-0 overflow-x-hidden relative">
      ...
      <div className="sticky top-0 z-[100] bg-indigo-600 dark:bg-indigo-800 py-3 px-4 md:px-6 text-white w-full flex flex-wrap md:flex-nowrap items-center justify-between border-b-4 border-indigo-700 shadow-md backdrop-blur-md">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto mb-3 md:mb-0">
          <button
            ...
            className="text-[10px] md:text-xs font-black bg-white/10 hover:bg-white/20 px-3 py-1.5 rounded-md transition-colors uppercase border border-white/20 shrink-0"
          >
            Volver
          </button>
        ...
        <div className="flex gap-3 items-center justify-between w-full md:w-auto">
          ...
          <div className="flex gap-3 items-center bg-black/30 px-4 py-1.5 rounded-lg border border-white/10 shadow-inner">
            <div className="text-center flex flex-col items-center">
              ...
              <span className="text-[8px] uppercase font-bold opacity-70 tracking-widest mt-0.5">
                Nota
              </span>
            </div>
      ...
      <div
        className={`flex-1 w-full m-0 p-0 ${isSummaryOnly ? "flex items-center justify-center" : ""}`}
      >
        {!isSummaryOnly && (
          <div className="w-full flex flex-col m-0 p-0">
            {result.questions.map((q: any, idx: number) => {
              ...
              const isSegment =
                visual &&
                (visual.type === "segmentos" || visual.theme === "segmentos");

              return (
                <div
                  key={q.id}
                  className={`w-full border-b dark:border-slate-800 p-5 md:p-8 bg-white dark:bg-slate-900 ${!isCorrect ? "border-l-8 border-l-red-500" : "border-l-8 border-l-emerald-500"}`}
                >
                  <div className="flex items-center gap-3 mb-2">
                    <span
                      className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black text-white shadow-sm ${isCorrect ? "bg-emerald-500" : "bg-red-500"}`}
                    >
                      {idx + 1}
                    </span>
                    <span
                      className={`text-xs font-black uppercase tracking-widest ${isCorrect ? "text-emerald-600" : "text-red-500"}`}
                    >
                      {isCorrect ? "Correcto" : "Fallaste"}
                    </span>
                  </div>

                  {/* 🔥 FIX VISUAL: Eliminado mb-4, pegado al máximo con leading-none y mb-2 */}
                  <div
                    className="text-slate-900 dark:text-slate-100 font-bold leading-tight mb-2 transition-all duration-200"
                  ...
                  {visual && (visual.theme || visual.type) && (
                    <div className="w-full max-w-4xl bg-white dark:bg-slate-900 rounded-xl border-2 border-slate-200 dark:border-slate-700 mb-4 overflow-hidden shadow-sm flex items-center justify-center">
                      <div
                        className={`w-full flex items-center justify-center ${isSegment ? "h-32 md:h-40" : "aspect-[21/6]"}`}
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-5 gap-2 md:gap-3 mb-4 w-full">
                    {["A", "B", "C", "D", "E"].map((letter) => {
                      const val =
                        options[letter] ?? options[letter.toLowerCase()];
                      if (val === undefined || val === null) return null;

                      const isUserChoice =
                        String(userAnsKey).toUpperCase() === letter;
                      ...
                      return (
                        <div
                          key={letter}
                          className={`px-3 py-1.5 rounded-lg border-2 flex flex-row items-center gap-3 transition-all ${
                            isUserChoice
                              ? isCorrect
                        ...
                        >
                          <span className="font-black opacity-50 text-sm shrink-0">
                            {letter})
                          </span>
                          <span className="font-bold flex-1">
                            <Latex>{String(val)}</Latex>
                          </span>
                  ...
                  {!isCorrect && (
                    <div className="mt-4 p-5 bg-indigo-50/80 dark:bg-indigo-900/10 rounded-xl border-2 border-indigo-100 dark:border-indigo-900/30 shadow-inner">
                      <div className="flex items-center gap-2 mb-3 text-indigo-700 dark:text-indigo-400 font-black text-xs uppercase tracking-widest">
                        <Sparkles size={16} />
                        {/comunic|lenguaj|verbal|lect|historia|sociales/i.test(
                          q.subtipo || "",
                        )
                          ? "Mentor Albus Dumbledore 🧙‍♂️"
                          : "Arquitecto Euclides 📐"}
                      </div>
                      <div
                        className="text-slate-800 dark:text-slate-200 font-medium leading-relaxed transition-all duration-200 whitespace-pre-wrap"
                        style={{ fontSize: `${fontSize}px` }}
                      >
